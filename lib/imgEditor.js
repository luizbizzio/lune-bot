...
const { isRestTypeNode } = require('typescript');
...
const convertImagesToVideo = async(pathn, colors) => {
  try {
    /*
    var images = [];
    for (let i = 1; i < colors.length; i++) {
      images.push(pathn+(i+1)+'.png');
    };
    */
...
const createAttpImg = async(_text, client, message, { mess, lang }) => {
  try {
    var pdff = Math.random().toString(36).substring(7);
    var ccanvas = cvs.createCanvas(500, 500);
    ...
    do {
      fontsize--;
      cctx.font = fontsize+"px Bungee"
      cfrt = fontsize+"px Bungee";
    } while(cctx.measureText(_text).width > ccanvas.width);

    var colors = ["#f12333", "#189ff1", "#fa1fff", "#ff1f", "#1ff812"];
    for (i in colors) {
      const canvas = cvs.createCanvas(500, 500);
      const ctx = canvas.getContext('2d');
      ...
      const out = fs.createWriteStream(`./media/frames/${pdff}-attp-${i}.png`);
      const stream = canvas.createPNGStream();
      stream.pipe(out);
      if (i >= colors.length - 1) {
        out.on('finish', async () => {
          //exec(`ffmpeg -r ${colors.length} -i ./media/frames/${pdff}-attp-%0d.png -c:v libx264 -pix_fmt yuv420p "./media/${pdff}.mp4"`, async function(err) {
          exec(`ffmpeg -r ${colors.length} -i ./media/frames/${pdff}-attp-%0d.png -vcodec png "./media/${pdff}.mp4"`, async function(err) {
          //exec(`python3 ./lib/frames2mp4.py "media/frames/${pdff}-attp-%0d.png" "media/${pdff}.mp4" ${colors.length}`, async function(err, stdout, stderr) {
            if (err) {
              console.log(err);
              client.reply(message.from, mess[lang].somethingWentWrong(), message.id);
            } else {
              await client.sendMp4AsSticker(message.from, `./media/${pdff}.mp4`, null, mess[lang].stickerMetadataVideo(10, "00:00:05.0", false, true));
              fs.unlinkSync(`./media/${pdff}.mp4`);
              for (i in colors) {
                fs.unlinkSync(`./media/frames/${pdff}-attp-${i}.png`);
              };
            };
          });
        });
