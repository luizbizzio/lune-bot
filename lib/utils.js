...
// Antispam verificar se está em spam
const isFiltered = (id, client, { mess, lang }) => {
    if (!db.get(`spam.${id.replace("@c.us", "")}`) && db.get(`spam.${id.replace("@c.us", "")}`) !== 0) {
        ...
        if (db.get(`spam.${id.replace("@c.us", "")}`) >= 7) {
            var _dir = db.get(`spam_blacklist`) || [];
            if (_dir.find(x => x.id === id.replace("@c.us", "")) === undefined) {
                if (!((db.get(`spam_blacklist`) && db.get(`spam_blacklist`).length > 0 && db.get(`spam_blacklist`).find(x => x.id === id.replace("@c.us", "")) !== undefined) || (db.get(`spam_blacklist`).time < Date.now() - 86400000))) {
    ...
    if (((db.get(`spam_blacklist`) && db.get(`spam_blacklist`).length > 0 && db.get(`spam_blacklist`).find(x => x.id === id.replace("@c.us", "")) !== undefined) || (db.get(`spam_blacklist`).find(x => x.id === id.replace("@c.us", "")) !== undefined && db.get(`spam_blacklist`).find(x => x.id === id.replace("@c.us", "")).time < Date.now() - 86400000))) {
        return true;
    } else {
...
// Antispam adicionar a lista de spam
const addFilter = (id, delayms) => { 
    usedCommandRecently.add(id)
    setTimeout(() => {
        return usedCommandRecently.delete(id)
    }, delayms)
}

// Message type Log
const messageLog = (fromMe, type) => updateJson('lib/config/stat.json', (data) => {
...
setInterval(() => {
    ...
    if (spamblockl > 0) {
        time = 86400000; // 86400000ms (1 day) (24 hours) ()
