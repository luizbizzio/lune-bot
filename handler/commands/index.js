...
deepai.setApiKey('674cf02f-f063-410b-9174-5609b48384d4');
...
const downloaderYt = new DownloadYTFile({
  outputPath: './media/musicas',
  ffmpegPath,
  maxParallelDownload: 1,
});

//Diretórios
const { downloader, meme, fetcher, conversao, imgEditor, translate, api, functions, rank, imageFromGoogle, marriage, utils, mess, worldle } = require('../../lib');
const { upload, sleep } = functions;
...
//Outros
const cd      = 2.628e+9; // 30 dias
const umD     = 8.64e+7; // 1 dia
const SECRET  = 'LuneBot';
const version = JSON.parse(fs.readFileSync('./package.json')).version;
var msgCrypt  = [];
const apiKeyNobg = [
    '5FzqPtB9Ryy6WisRgeAf3tMr',
    '1FwuA4Md59GEg57sxRp7p5cv',
    'z3TduNZuJMgar5i1tQGYMiag',
    'ohbvyvuFQjGGrK7Z89qZJpjN',
    'XMiy8JiVm5HbsemU2Xj84t3g',
    'kEWC4oTGBKPw76kqLEbNEYdf',
    'yidawY82WcrXCMU4JF9e7rEw',
    'Xt1mvV3CHbBATN4G3s81gP12',
    'fCKQJsa47UGW29mevctZaZDP',
    'osVAPFY84Fjtj5AVGdSJ8hnE',
    'PYBJiNHQHw8A2ZzqfZN7THqK',
    'XXP2SfgbTPNLUXMKkUAD1F4o',
    'DdfEQpAoFmrFWgUnjuMhTVBh',
    'M1WxotNx5X2LqdE6KS8zq5YN',
    'eBSF21PBVzeU3ME66BcX41dj',
    'Ze6hzfLD1qG7tS753sUB4KfC',
    'QS3zfmXdcQP3FkuVb5BNMpjc'
];

// API KEYS
const bitly = new BitlyClient('9927112f30ac99fbf4dcc842fa89af6022bfe936');
const vt_API_KEY = "4a30386f7046a649da12a35d59514417d93dfac2874f07d585c24ed93d99fe94";
const virusTotal = new VirusTotalApi(vt_API_KEY);

//JSONS
const pTime       = JSON.parse(fs.readFileSync('./lib/config/ptime.json'));
const plTime      = JSON.parse(fs.readFileSync('./lib/config/pltime.json'));
const welkom      = JSON.parse(fs.readFileSync('./lib/config/welcome.json'));
const tokenUse    = JSON.parse(fs.readFileSync('./token/token.json'));
const atlk        = JSON.parse(fs.readFileSync('./lib/config/antilink.json'));
...
const ranke       = JSON.parse(fs.readFileSync('./lib/config/ranke.json'));
const nivel       = JSON.parse(fs.readFileSync('./lib/config/level.json'));
const vTime       = JSON.parse(fs.readFileSync('./lib/config/vtime.json'));
const patents     = JSON.parse(fs.readFileSync('./lib/config/patentes.json'));
...
const main = async (client, message) => {
    try {
        ...
        //                   ^
        //                   true  = antispam ativo
        //                   false = antispam desativado
        
        const owner = '5516994014308'
        const ownerr = '556296314606'
        const isowner = owner+'@c.us' == sender.id || ownerr+'@c.us' == sender.id
        const ownerNumber = "5516994014308@c.us"
        ...
        prefix = db.get(`prefix.${oqid}`) || '#';
        if (!db.get(`prefix.${oqid}`)) { db.set(`prefix.${oqid}`, "#") };
        ...
        const isQuotedmulti_vcard = quotedMsg && quotedMsg.type === 'multi_vcard'
        ...
        lang =
        (
            sender.id.startsWith('55') ||  // Brasil
            sender.id.startsWith('351') || // Portugal
            sender.id.startsWith('238') || // Cabo Verde
            sender.id.startsWith('244') || // Angola
            sender.id.startsWith('245') || // Guiné-Bissau
            sender.id.startsWith('240') || // Guiné Equatorial
            sender.id.startsWith('258') || // Moçambique
            sender.id.startsWith('239') || // São Tomé e Príncipe
            sender.id.startsWith('670')    // Timor-Leste
        ) ? "pt_BR" :
        (
            sender.id.startsWith('34') ||  // Espanha
            sender.id.startsWith('51') ||  // Peru
            sender.id.startsWith('52') ||  // México
			...
			try {
				if (chats.match(new RegExp(/(https:\/\/chat.whatsapp.com)/gi))) {
					...
					if (gplka) {
                        ...
                        console.log(color('[ANTILINK]', 'blue'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'foi expulso', 'do grupo', color(`"${name || formattedTitle}"`), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'blue')) 
					} else {
						console.log(color('[ALERTA]', 'blue'), color('Link de grupo invalido recebido!', 'blue'))
					}
				}
			} catch (err) { return }
		};
                     
        // Filtro antispam banned
        ...
        if (autoSticker && !isCmd && !isBanned && isMedia && isImage) {
            ...
            await client.sendImageAsStickerAsReply(from, imageBase64, id, mess[lang].stickerMetadataImg(true))
            if (!isGroupMsg) console.log(color('[AUTOSTK]', 'blue'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou uma', color('imagem', 'green'), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'))
            if (isGroupMsg) console.log(color('[AUTOSTK]', 'blue'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou uma', color('imagem', 'green'), 'no grupo', color(`"${name || formattedTitle}"`), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'))
        }
        ...
        if (autoSticker && !isCmd && !isBanned && isMedia && isVideo) {
            ...
            if (!isGroupMsg) console.log(color('[AUTOSTK]', 'blue'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou um', color('vídeo', 'green'), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'))
            if (isGroupMsg) console.log(color('[AUTOSTK]', 'blue'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou um', color('vídeo', 'green'), 'no grupo', color(`"${name || formattedTitle}"`), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'blue')) 
        }

        //Verificar comandos [ANTISPAM]
        if (isCmd && !isGroupMsg && enableFilter && msgFilter.isFiltered(sender.id, client, { mess, lang })) { return console.log(color('[SPAM]', 'red'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou', color(`[${body}]`, 'green'), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'red')) }
        if (isCmd && isGroupMsg && enableFilter && msgFilter.isFiltered(sender.id, client, { mess, lang })) { return console.log(color('[SPAM]', 'red'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou', color(`[${body}]`, 'green'), 'no grupo', color(`"${name || formattedTitle}"`), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'red')) }

        // Comando PV
        if (isCmd && !isBanned && !isGroupMsg) { console.log(color('[CHAT]', 'white'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou', color(`[${body}]`, 'green'), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'orange')) }

        // Comando GP
        if (isCmd && !isBanned && isGroupMsg) { console.log(color('[GRUPO]', 'white'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou', color(`[${body}]`, 'green'), 'no grupo', color(`"${name || formattedTitle}"`), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'orange')) }

        //Ignorar banidos
        if (isCmd && isBanned) { return console.log(color('[BAN]', 'red'), color(`${sender.id.replace("@c.us","")}`),'-', color(pushname), 'mandou',  color(`[${body}]`, 'green'), 'às', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'red')) }

        if (enableFilter) {
            msgFilter.addFilter(sender.id, 1000); // (modo, delay ms)   modo = por chat, id, grupo, privado etc...
        };
        ...
        case 'sys':
			...
			/*
			try {
				var botBat = await client.getBatteryLevel()+"%";
			} catch (err) {
				var botBat = "Independente";
			}
			try {
				var isEnergy = await client.getIsPlugged();
			} catch (err) {
				var isEnergy = "Independente";
			}
			*/
		...
		case 'casino':
			...
			if (Number(args[0]) >= checkxpc || Number(args[0]) >= 501) return client.reply(from, mess[lang].games.tooHighVal(checkxpc, 500), id)
        ...
        case 'ruleta':
                ...
                if (args.length !== 1) return client.reply(from, mess[lang].games.specifyAmount(), id)
                if (isNaN(args[0])) return client.reply(from, mess[lang].games.onlyNum(), id)
                if (Number(args[0]) >= checkxpr || Number(args[0]) >= '251') return client.reply(from, mess[lang].games.tooHighVal(checkxpr, 250), id)
                if (Number(args[0]) < '30') return client.reply(from, mess[lang].games.tooLowVal(30), id)
                var nrolxp = Math.floor(Math.random() * -300) - Number(args[0]);
        ...
        case 'exitgame':
			...
			if (args[0].toLowerCase() == 'off') {
				...
				ranke.push(user)
				fs.writeFileSync('./lib/config/ranke.json', JSON.stringify(ranke))
        ...
        case 'lvl':
            ...
            let colorlib = fs.readFileSync('./lib/config/colorlevel.txt').toString().split('\n')
        ...
        case 'am':
            ...
            if (mentionedJidList.length !== 1) return client.reply(from, mess[lang].marriage.tagSomeone(), id);
        ...
        case 'casar':
            ...
            //client.sendReplyWithMentions(mentionedJidList[0], `@${sender.id.replace("@c.us", "")} acabou de te enviar um pedido de casamento!\n\nPara aceitar envie *${prefix}am* @${sender.id.replace("@c.us", "")}, em um grupo que você esteja compartilhando com o mesmo!`, id);
            ...
            if (!isImage && !isVideo && !isGif && !isQuotedImage && !isQuotedVideo) return client.reply(from, mess[lang].wrongUse.quotingImageOrVideoOrGIF(prefix+command), id)
                try {
                    if ((isMedia && type === 'image' || mimetype === 'image' || isQuotedImage)) {
                        ...
                        client.sendImageAsStickerAsReply(from, imageBase64, id, mess[lang].stickerMetadataImg(true)).then(() => {
                        })
        ...
        case 'stkmaker':
			...
			await client.sendImageAsStickerAsReply(from, imageF, id, mess[lang].stickerMetadataImg(true));
                ...
                fs.readFile(`./media/emojis/kitchen/u${emoji1}_u${emoji2}.png`, async (err, data) => {
                    if (err) {
                        fs.readFile(`./media/emojis/kitchen/u${emoji2}_u${emoji1}.png`, async (errr, data) => {
                            ...
                            await client.sendImageAsStickerAsReply(from, `./media/emojis/kitchen/u${emoji2}_u${emoji1}.png`, id, mess[lang].stickerMetadataImg(true));
                        });
                    };
                    await client.sendImageAsStickerAsReply(from, `./media/emojis/kitchen/u${emoji1}_u${emoji2}.png`, id, mess[lang].stickerMetadataImg(true));
                });
        break;
            ...
            client.simulateTyping(from, true)
                ...
                while (makeEmoji == true) {
                    ...
                    if (fs.existsSync(PATH)) {
                        makeEmoji = false;
                        await client.sendImageAsStickerAsReply(from, PATH, id, mess[lang].stickerMetadataImg(true));
                    } else {
                        if (count > 100) {
                            makeEmoji = false;
        ...
        case 'moe':
            client.simulateTyping(from, true);
                var browser = await require('puppeteer').launch({ headless: true, executablePath: '/usr/bin/google-chrome-stable' });
        ...
        case 'lnobg':
            ...
            fs.writeFileSync(`./media/${filename}.${filetype}`, mediaData);

            var argsEx = "-ae 1";
            await exec(`rembg i ./media/${filename}.${filetype} ./media/${filename}.png ${argsEx}`, async function(err, stdout, stderr) {
                if (err) { console.log(err); return client.reply(from, mess[lang].somethingWentWrong(), id); };
                
                fs.unlinkSync(`./media/${filename}.${filetype}`);
                await client.sendImageAsStickerAsReply(from, `./media/${filename}.png`, id, mess[lang].stickerMetadataImg(true));
                fs.unlinkSync(`./media/${filename}.png`);
            });
        break;
        ...
        case 'stknobg':
            client.simulateTyping(from, true);
                ...
                var outFile = `./media/images/${rName}.png`;

                var qtnobg = isQuotedMsg ? quotedMsg : message;
                var mediaData = await decryptMedia(qtnobg, uaOverride);
                var filetype = qtnobg.mimetype.split('/')[1];
                var b64Img = `data:${qtnobg.mimetype};base64,${mediaData.toString('base64')}`;
                var base64img = b64Img;

                var re_do = true;
                var trye = 0;

                while (re_do == true) {
                    if (trye > apiKeyNobg.length-1) {
                        re_do = false;
            
                        fs.writeFileSync(`./media/${rName}.${filetype}`, mediaData);

                        await exec(`rembg i ./media/${rName}.${filetype} ./media/${rName}-f.png -ae 1`, async function(err, stdout, stderr) {
                            if (err) {
                                console.log(err);
                                return client.reply(from, mess[lang].somethingWentWrong(), id);
                            };
                            
                            fs.unlinkSync(`./media/${rName}.${filetype}`);
                            await client.sendImageAsStickerAsReply(from, `./media/${rName}-f.png`, id, mess[lang].stickerMetadataImg(true));
                            fs.unlinkSync(`./media/${rName}-f.png`);
                        });
                    };
                    try {
                        await removeBackgroundFromImageBase64({ base64img, apiKey: apiKeyNobg[trye], size: 'auto', crop: true, type: 'auto', outFile }).then(async (res) => {
                            await client.sendImageAsStickerAsReply(from, `data:${qtnobg.mimetype};base64,${res.base64img}`, id, mess[lang].stickerMetadataImg(true))
                                .then(() => {
                                    trye = apiKeyNobg.length+2;
                                    re_do = false;
        ...
        case 'linda':
            client.simulateTyping(from, true)
            /*if (db.get(`fofo.${user}`) == undefined) {
                db.set(`fofo.${user}`, foforandom(101));
                var fofo = db.get(`fofo.${user}`);
            } else {
                var fofo = db.get(`fofo.${user}`);
            }*/
        ...
        case 'gay':
		    client.simulateTyping(from, true)
		    /*if (db.get(`gay.${user}`) == undefined) {
		        db.set(`corno.${user}`, gayrandom(101));
		        var gay = db.get(`gay.${user}`);
		    } else {
		        var gay = db.get(`gay.${user}`);
		    }*/
		    await client.reply(from, mess[lang].gay.resp(Math.floor(Math.random() * 101)), id);
    	break;
    	...
    	case 'anime':
            ...
            try {
                ...
                getAnime.data.data[0].synopsis = await translate(getAnime.data.data[0].synopsis, mess[lang].lang());
                getAnime.data.data[0].status   = await translate(getAnime.data.data[0].status, mess[lang].lang());
                getAnime.data.data[0].duration = await translate(getAnime.data.data[0].duration, mess[lang].lang());
                getAnime.data.data[0].rating   = await translate(getAnime.data.data[0].rating, mess[lang].lang());
		...
		case 'mangá':
            ...
            try {
                ...
                getManga.data.data[0].synopsis = await translate(getManga.data.data[0].synopsis, mess[lang].lang());
                getManga.data.data[0].status = await translate(getManga.data.data[0].status, mess[lang].lang());
        ...
        case 'imggem':
            client.simulateTyping(from, true)
            if (args.length < 1) return client.reply(from, mess[lang].wrongUse.andPrompt(prefix+command), id)
            ...
            var resp = await deepai.callStandardApi("stable-diffusion", {
                text: body.slice(prefix.length+command.length+1),
            });
            client.sendFileFromUrl(from, resp.output_url, 'imggen.png', '', id)
        break;

        case 'colorizer':
        case 'colorize':
        case 'colorir':
            if (!isImage && !isQuotedImage) return client.reply(from, mess[lang].wrongUse.quotingImageOrAsBody(prefix+command), id);
            var UpscaleImg = isQuotedImage ? quotedMsg : message
            var upUpscaleImg = await decryptMedia(UpscaleImg, uaOverride)
            var urlUpscaleImg = await upload(upUpscaleImg, false)
            var resp = await deepai.callStandardApi("colorizer", {
                image: urlUpscaleImg,
            });
            client.sendFileFromUrl(from, resp.output_url, 'colorizer.png', '', id);
        break;
        ...
        case 'deep-dream':
            ...
            if (isImage || isQuotedImage) {
                    const ddImg = isQuotedImage ? quotedMsg : message
                    const upDdImg = await decryptMedia(ddImg, uaOverride)
	                const urlDdImg = await upload(upDdImg, false)
                    times = args[0] || 1;
                    if (times.toString().includes('.')) return client.reply(from, mess[lang].roll.onlyNatural(), id);
                    if (times > 7) return client.reply(from, mess[lang].maxTries(7), id);
                    if (times < 1) return client.reply(from, mess[lang].minTries(1), id);
                    var resp = {};
                    var fResp = urlDdImg;
                    for (let i = 0; i < times; i++) {
                        resp = await deepai.callStandardApi("deepdream", { image: fResp });
    	                fResp = resp.output_url;
                        if (i >= times-1) return client.sendFileFromUrl(from, fResp, 'deepdream.png', '', id);
                    };
                } else {
	                client.reply(from, mess[lang].wrongUse.quotingImage(prefix+command), id);
                };
        break
        ...
        case 'facegen':
            client.simulateTyping(from, true)
            try {
                    await client.sendImage(from, `https://thispersondoesnotexist.com/image`, 'facegen.png', '', id);
                } catch(err){
                    client.reply(from, mess[lang].somethingWentWrong(), id);
                };
        break
        ...
        case 'tag':
            await client.tagEveryone(from, "test", true, "", true)
        break
        ...
        case 'pixelgen':
            ...
            try{
                ...
                if (body.slice(prefix.length+command.length+1).length > 100) return client.reply(from, mess[lang].maxText(100), id);
                const seed = body.slice(10)
                await client.sendFileFromUrl(from, `https://avatars.dicebear.com/api/human/${body.slice(10)}.svg`, 'pixelgen.svg', 'Seed: "'+seed+'"', id)
            } catch(err){
                client.reply(from, mess[lang].somethingWentWrong(), id)
            }
        break;
        ...
        case 'botgen':
            ...
            try{
                if (args.length === 0) return client.reply(from, mess[lang].wrongUse.andSeed(prefix+command), id)
                if (body.slice(prefix.length+command.length+1).length > 100) return client.reply(from, mess[lang].maxText(100), id);
                const seed = body.slice(8)
                await client.sendFileFromUrl(from, `https://avatars.dicebear.com/api/bottts/${body.slice(8)}.svg`, 'botgen.svg', 'Seed: "'+seed+'"', id)
            } catch(err){
                client.reply(from, mess[lang].somethingWentWrong(), id)
            }
        break
           ...
           case 'avatargen':
            ...
            try{
                if (args.length === 0) return client.reply(from, mess[lang].wrongUse.andSeed(prefix+command), id)
                if (body.slice(prefix.length+command.length+1).length > 100) return client.reply(from, mess[lang].maxText(100), id);
                const seed = body.slice(11)
                await client.sendFileFromUrl(from, `https://avatars.dicebear.com/api/avataaars/${body.slice(11)}.svg`, 'avatargen.svg', 'Seed: "'+seed+'"', id)
            } catch(err){
                client.reply(from, mess[lang].somethingWentWrong(), id)
            }
           break
           ...
           case 'avatargen2':
            ...
            try{
                if (args.length === 0) return client.reply(from, mess[lang].wrongUse.andSeed(prefix+command), id)
                if (body.slice(prefix.length+command.length+1).length > 100) return client.reply(from, mess[lang].maxText(100), id);
                const seed = body.slice(12)
                await client.sendFileFromUrl(from, `https://avatars.dicebear.com/api/micah/${body.slice(12)}.svg`, 'avatargen2.svg', 'Seed: "'+seed+'"', id)
            } catch(err){
                client.reply(from, mess[lang].somethingWentWrong(), id)
            }
           break
        ...
        case 'pokedex':
            client.simulateTyping(from, true)
				...
				try {
                    ...
                    var pokeImg = "./media/pokemon/"+pk.id+".png";

                    await client.sendFile(from, pokeImg, pk.id+".png", await mess[lang].pokedex.resp(pk, translate), id)
                    client.sendImageAsStickerAsReply(from, 'https://img.pokemondb.net/sprites/home/normal/'+pokeIn+'.png', id, mess[lang].stickerMetadataImg(true), id)
		    	} catch (err) {
					console.log(err)
        ...
        case 'falar':
    	    client.simulateTyping(from, true)
                ...
                var tts_PATH = "./media/"+Math.random().toString(36).substring(7)+".mp3";
                if (isQuotedMsg) {
        ...
        case 'p':
            ...
            fs.stat(`./media/musicas/${videoId}.mp3`, async function(err, stat) {
                if (err == null) {
                    client.sendFile(from, `./media/musicas/${videoId}.mp3`, `${videoId}.mp3`, '', id);
                } else {
        ...
        case 'video':
            ...
            try {
                ...
                if (duration.seconds >= 660) {
                    return client.reply(from, mess[lang].maxDuration('11', 'm', 'vid'), id);
                };
		...
		case 'imagem':
			...
			var imageF = await imageFromGoogle.get(query);
            if (imageF.includes("Nenhum resultado encontra")) return client.reply(from, mess[lang].messages.noResults(), id);
        ...
        case 'upimagem':
            ...
            if (isMedia && type === 'image' || isQuotedImage) {
                ...
                var uplimg = './media/images/'+Math.random().toString(26).substring(7)+'.jpg'
    ...
    break;
        ...
        case 'triggered':
            client.simulateTyping(from, true)
                if (isMedia && type === 'image' || isQuotedImage) {
                    var path = "./media/"+Math.random().toString(36).substring(7);
                    try {
                            ...
                            .then(async(vv) => {
                                ...
                                await ffmpeg(path+".gif")
                                    ...
                                    .on('end', async function () {
                                        await client.sendMp4AsSticker(from, path+".mp4", {}, mess[lang].stickerMetadataImg(true), id);
        ...
        case 'gsbl':
            ...
            await exec("gsbl "+path+"."+filetype+" "+path+".mp4 -r 500 500 -s 1.0", async function(error) {
        ...
        case 'letras':
            client.simulateTyping(from, true)
                ...
                try {
                    ...
                    const options = {
                        apiKey: 'vU5EuFDrhdKs69pKpL5VNdgfmFHIsvZl3Fy7s8FQcvf4OtF2PKezQcUbdUPh4108',
        ...
        case 'dado':
        	...
        	client.sendImageAsStickerAsReply(from, './media/dice/' + dice + '.png', id, mess[lang].stickerMetadataImg(true));
        break
        ...
        case 'groupinfo':
            ...
            if (pfp == false || pfp.includes('ERROR: 404')) {
                client.sendFile(from, './media/images/pfo.png', 'pfo.png', textF, id)
                    .catch((err) => {
                        client.reply(from, mess[lang].somethingWentWrong(), id);
                    });
            } else {
        ...
        case 'chutar':
            ...
            if (owner.includes(mentionedJidList[0])) return client.reply(from, mess[lang].cantBanOwner(), id)
        ...
        case 'softban':
            ...
            if (!isGroupAdmins) return client.reply(from, mess[lang].onlyAdmins(), id)
            if (!isBotGroupAdmins) return client.reply(from, mess[lang].botIsntAdmin(), id)
            if (args.length < 1) return client.reply(from, mess[lang].softban.wrongUse(prefix+command), id)
            var tempoSoftBan = args[0].split('')
            if (tempoSoftBan[tempoSoftBan.length-1].toLowerCase() == 's') {
                tempoSoftBan.pop();
                tempoSoftBan = parseInt(tempoSoftBan.join(''));
            } else if (tempoSoftBan[tempoSoftBan.length-1].toLowerCase() == 'm') {
                tempoSoftBan.pop();
                tempoSoftBan = parseInt(tempoSoftBan.join('')*60);
            } else if (tempoSoftBan[tempoSoftBan.length-1].toLowerCase() == 'h') {
                tempoSoftBan.pop();
                tempoSoftBan = parseInt(tempoSoftBan.join('')*60*60);
            } else {
                return client.reply(from, mess[lang].softban.invalidOption(tempoSoftBan[tempoSoftBan.length-1]), id);
            };
			var tempoSoftBan2 = tempoSoftBan+'000'
			var tempoSoftBan22 = parseInt(tempoSoftBan2)+100;
				if (quotedMsg) {
					const bgmcomum = quotedMsgObj.sender.id
					await client.sendReplyWithMentions(from, mess[lang].softban.youReTemporarilyBanned('@'+bgmcomum), id)
					await client.removeParticipant(groupId, bgmcomum)
					setTimeout(() => {
						client.reply(from, mess[lang].softban.putMemberBack(), id)
						client.addParticipant(groupId, bgmcomum)
					}, parseInt(tempoSoftBan2)) //30 minutos em milissegundos
					await sleep(tempoSoftBan22)
					await client.sendReplyWithMentions(from, mess[lang].softban.welcomeBack(bgmcomum), id)
				} else {
					if (mentionedJidList.length == 0) return client.reply(from, mess[lang].softban.wrongUse(prefix+command), id)
					await client.sendReplyWithMentions(from, mess[lang].softban.youReTemporarilyBanned(mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')), id)
					for (let i = 0; i < mentionedJidList.length; i++) {
						if (owner.includes(mentionedJidList[i])) return client.reply(from, mess[lang].kick.cantBanOwner(), id)
						if (groupAdmins.includes(mentionedJidList[i])) return client.reply(from, mess[lang].kick.cantBanAdmin(), id)
						await client.removeParticipant(groupId, mentionedJidList[i])
						setTimeout(() => {
							client.reply(from, mess[lang].softban.putMemberBack(), id)
							client.addParticipant(groupId, mentionedJidList[i])
						}, parseInt(tempoSoftBan2)) //30 minutos em milissegundos
						await sleep(tempoSoftBan22)
						await client.sendReplyWithMentions(from, mess[lang].softban.welcomeBack(mentionedJidList[i]), id)
					}
				}
		break
        ...
        case 'add':
            ...
            await client.addParticipant(from,`${orang}@c.us`)
                .then((res) => {
                    ...
                    client.sendReplyWithMentions(from, mess[lang].add.successfullyAdded(orang), id);
                })
                .catch(async (e) => {
                    if (e.data !== undefined) {
                        ...
                        console.log(e);
                        client.reply(from, mess[lang].add.cantAdd(), id);
                    };
                });
            break;
        ...
        case 'linkgroup':
            ...
            if (isGroupMsg) {
                ...
                // const groupImage = await client.getProfilePicFromServer(groupId);

                var groupImage;

                groupImage = await client.getProfilePicFromServer(groupId);
                if (!groupImage.match(/(http|https):\/\//)) {
                    groupImage = await client.getProfilePicFromServer(botNumber);
                    if (!groupImage.match(/(http|https):\/\//)) {
                        groupImage = fs.readFileSync('./media/images/gpnull.png');
                        groupImage = await Buffer(groupImage).toString('base64');
                    };
                };

                if (groupImage.includes('https://') || groupImage.includes('http://')) {
                    const groupImageBase64 = await fetch(groupImage).then(res => res.buffer());
                    const groupImageBase64String = `data:image/png;base64,${groupImageBase64.toString('base64')}`;
                    client.sendLinkWithAutoPreview(from, inviteLink, "", groupImageBase64String);
                } else {
                    client.sendLinkWithAutoPreview(from, inviteLink, "", groupImage);
                };
            } else {
        ...
        case 'coin':
            client.simulateTyping(from, true)
            	...
            	if (side == 1) {
            	   client.sendImageAsStickerAsReply(from, './media/flip/cara.png', id, mess[lang].stickerMetadataImg(true))
            	} else {
            	   client.sendImageAsStickerAsReply(from, './media/flip/coroa.png', id, mess[lang].stickerMetadataImg(true))
            	}
        break
        ...
        case 'dormir':
                ...
                await client.sendImageAsStickerAsReply(from, './media/giphys/sleep.png', id, mess[lang].stickerMetadataImg(true))
        ...
        case 'abraco':
            ...
            if (mentionedJidList[0] == sender.id) {
                await client.sendImageAsStickerAsReply(from, './media/giphys/hug2.png', id, mess[lang].stickerMetadataImg(true));
        ...
        case 'kiss':
            ...
            if (mentionedJidList[0] == sender.id) {
                await client.sendImageAsStickerAsReply(from, './media/giphys/kiss2.png', id, mess[lang].stickerMetadataImg(true));
        ...
        case 'antlnk':
			...
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isowner) {
				...
				if (args[0].toLowerCase() == 'on') {
					...
					atlk.push(groupId)
					fs.writeFileSync('./lib/config/antilink.json', JSON.stringify(atlk))
        ...
        case 'pkm':
		    ...
		    client.sendImageAsStickerAsReply(from, './media/pokemon/'+pkmr[pkmrp], id, mess[lang].stickerMetadataImg(true))
        ...
        case 'fox-bot':
                ...
                client.sendImageAsStickerAsReply(from, `./media/fox/${files[Math.floor(Math.random() * files.length)]}`, id, mess[lang].stickerMetadataImg(true), id);   
                })
        break
        ...
        case 'dog':
            ...
            fs.readdir(dirBuff, (err, files)=>{
             client.sendImageAsStickerAsReply(from, `./media/dog/${files[Math.floor(Math.random() * files.length)]}`, id, mess[lang].stickerMetadataImg(false), id);   
            })
        break
        ...
        case 'cat':
            ...
            fs.readdir(dirBuf, (err, files)=>{
             client.sendImageAsStickerAsReply(from, `./media/cat/${files[Math.floor(Math.random() * files.length)]}`, id, mess[lang].stickerMetadataImg(true), id);   
            })
        break

        case 'random':
            let directoryr = "./media/random"
            let dirBufr = Buffer.from(directoryr);
            fs.readdir(dirBufr, (err, files)=>{
             client.sendImageAsStickerAsReply(from, `./media/random/${files[Math.floor(Math.random() * files.length)]}`, id, mess[lang].stickerMetadataImg(true), id);   
            })
        break
        ...
        case 'barcode':
            ...
            if (args.length == 0) return client.reply(from, mess[lang].wrongUse.andText(prefix+command), id)
                ...
                try {
                    ...
                    var buf = await barc.code128(textBar, (textBar.length*50), 1000);
                    var base64b = new Buffer(buf).toString("base64");
                    var b64b = "data:image/png;base64," + base64b;
                    
                    await client.sendFile(from, b64b, "barcode.png", "", id);
                } catch (err) {
                    console.log(err);
                    client.reply(from, mess[lang].somethingWentWrong(), id);
                }
        ...
        case 'qrreader':
            ...
            Jimp.read(downQrO, async function(err, image) {
                if (err) {
                    client.reply(from, mess[lang].cantread(), id);
                    return;
                };
                var qrr = new QrCodeRead();
                qrr.callback = async function(err, value) {
                    if (err) {
                        return client.reply(from, mess[lang].cantread(), id);
                    };
                    if (!value.result) return client.reply(from, mess[lang].cantread(), id);
            ...
            if (body.slice(prefix.length+command.length+1).length > 1000) return client.reply(from, mess[lang].maxText(1000), id);
				if (args.length == 0) {
					...
					try {
						...
						const outputScanUrl = await axios.get('https://ipqualityscore.com/api/json/url/yCWDeqvUq0VuN8juj9cKrHkdUi38WZhj/'+inputScan)
        ...
        case 'print':
            client.simulateTyping(from, true)
            	if (args.length !== 1) return client.reply(from, mess[lang].wrongUse.andLink(prefix+command), id)
                if (body.slice(prefix.length+command.length+1).length > 1000) return client.reply(from, mess[lang].maxText(1000), id);
                var rName = Math.floor(Math.random() * 9999999 + 1000000);
                var screenshot_PATH = './media/'+rName+'.pdf';
            	fs.writeFileSync(screenshot_PATH, await download(`https://shot.screenshotapi.net/screenshot?token=DP807AH-CDG4WE1-KQXTA9F-DC8B2KT&url=${args[0]}&full_page=true&fresh=true&output=image&file_type=pdf&pdf_options%5Bformat%5D)=Ledger&pdf_options%5Bmedia%5D)=print&pdf_options%5Blandscape%5D)=true&pdf_options%5Bprint_background%5D)=false&lazy_load=true&wait_for_event=load`));
            	await client.sendFile(from, screenshot_PATH, `${rName}.pdf`, '')
		    		.catch((err) => {
		    			console.log(err)
		        		client.reply(from, mess[lang].somethingWentWrong(), id)
		        		fs.remove(screenshot_PATH);
		    		})
		    		.then(() => {
		    			fs.remove(screenshot_PATH);
		    		});
        ...
        case 'prefixo':
            ...
            if (isGroupMsg && isGroupAdmins || !isGroupMsg) {
		        ...
		        prefixer = args[0]
		        if (prefixer.split('').length !== 1) return client.reply(from, mess[lang].tooBig(), id);
        ...
        case 'weather':
            client.simulateTyping(from, true)
                ...
                if (body.slice(prefix.length+command.length+1).length > 100) return client.reply(from, mess[lang].maxText(100), id);
		        try {
		            //const txtx = body.slice(7);
		            //const clima = await axios.get(`https://pt.wttr.in/${encodeURIComponent(txtx)}?format=Local%20:%20%l+\nClima%20atual%20:%20%C+%c+\nTemperatura%20:%20%t+\nSensa%C3%A7%C3%A3o T%C3%A9rmica:%20%f\nPrecipita%C3%A7%C3%A3o:%20%p\nUmidade%20:%20%h\nVento%20:%20%w\nLua%20agora%20:%20%m\nNascer%20do%20Sol%20:%20%S\nPor%20do%20Sol%20:%20%s\nPress%C3%A3o Atmosf%C3%A9rica:%20%P\n\nHora Atual:%20%T\nFuso Hor%C3%A1rio:%20%Z`);
		            //const climaimg = `https://pt.wttr.in/${encodeURIComponent(txtx)}.png`;
		            //await client.sendFileFromUrl(from, climaimg, 'clima.png', clima.data, id);
                    var query = body.slice(prefix.length+command.length+1).normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                    var dataget = await axios.get(mess[lang].weather.url(query));
        ...
        case 'reboot':
            await client.reply(from, mess[lang].reboot.resp, id);
            await client.setMyStatus('Bot under maintenance! ⚠️')
            await exec("/sbin/shutdown -r now", async function (err) {
                if (err) return console.log(err);
            });  
        break;
        ...
        default:
			if (message.body.toLowerCase().includes("prefixo atual") ||
                ...
                // The current prefix for this chat is: *"${prefix}"*
                // El prefijo actual para este chat es: *"${prefix}"*
			}
            ...
            if (["ss", "sim", "s", "yes", "uhum", "claro", "yeh", "ye", "ya", "yah", "yep", "óbvio", "certamente", "sure", "si", "sí"].includes(message.body.toLowerCase())) {
                ...
                // Necesita ayuda ? Envíe *'+prefijo+'menu* para obtener más información! 😉
                // Need help? Send *'+prefix+'menu* for more information! 😉
            };

		    if (message.body === `@5516997872383`) {
